        ...
        if ( mDB != null) {
            ...
            while (!cursor.isAfterLast()) {
                String jstr = "{'m_id':'%s','custom_name':'%s', 'addressbook':'%s','mobile':'%s',  'corps':%s," +
                        ...
                        "'contact_id':'%s', 'contact_key':'%s' , 'status_msg':'%s'}";
                String jstrfmt = String.format( jstr , cursor.getString(0), cursor.getString(1), cursor.getString(2), cursor.getString(3),cursor.getString(4),
...
//                Log.d(TAG , "unread " + unread );
    ...
    public static JSONObject chkContacts(JSONObject obj) throws JSONException {
        ...
        obj.put("updated_time", created_time );
